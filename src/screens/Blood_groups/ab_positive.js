...
import {signOut,bring_AB_Positive} from '../../../Store/actions/auth';
import {useDispatch,useSelector} from 'react-redux';

const AB_Positive = ({navigation})=> {
  ...
  const onRefresh = React.useCallback(() => {
    ...
    auth().onAuthStateChanged((user) => {
      if (user) {
        dispatch(bring_AB_Positive('AB+',user.uid));
      }
    });
    wait(1200);
  }, []);
  ...
  const getABpositive = useSelector(({auth}) => {
    return auth.abpositive;
  });
    return (
      ...
      <ScrollView
        refreshControl={
          ...
          {getABpositive
            && getABpositive.length >
              0 ? (
                getABpositive.map((item, index) => {
                  return (
